...
export function createTestRunnerTools(workingDirectory: string): Tool[] {
  const runTests: Tool = {
    name: 'runTests',
      ...
      try {
          ...
          if (jsonMatch) {
            ...
            testResult = {
              ...
              failures: (parsed.testResults || [])
                .flatMap((tr: { assertionResults: Array<{ status: string; fullName: string; failureMessages: string[] }>; name: string }) =>
                  (tr.assertionResults || [])
                    .filter((ar: { status: string }) => ar.status === 'failed')
                    .map((ar: { fullName: string; failureMessages: string[] }) => ({
                      name: ar.fullName,
                      file: tr.name,
        ...
        if (testResult) {
          ...
          return {
            success,
            output: `${summary}\n\n${result.stdout}`,
            metadata: testResult as unknown as Record<string, unknown>,
          }
        }
  ...
  const listTests: Tool = {
    ...
    async execute(input: unknown): Promise<ToolResult> {
      const { pattern = '**/*.test.{ts,tsx}' } = input as { pattern?: string }
  ...
  const checkTestFile: Tool = {
    ...
    async execute(input: unknown): Promise<ToolResult> {
      const { sourceFile } = input as { sourceFile: string }
