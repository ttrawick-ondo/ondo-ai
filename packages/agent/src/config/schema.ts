...
export function mergeWithDefaults(partial: Partial<AgentConfig>): AgentConfig {
...
export function validatePartialConfig(input: unknown): ConfigValidationResult {
  ...
  try {
    ...
    if (result.success) {
      const merged = mergeWithDefaults(result.data as Partial<AgentConfig>)
      return {
        success: true,
        config: merged,
      }
