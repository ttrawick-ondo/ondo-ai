    ...
    program
        ...
        try {
                    ...
                    for (const toolUse of result.toolsUsed) {
                        if (toolUse.toolName === 'runTypeCheck') {
                            results.typeCheck = {
                                ...
                                issues: toolUse.result.metadata?.errorCount || 0,
                            };
                        }
                        else if (toolUse.toolName === 'runEslint') {
                            const meta = toolUse.result.metadata;
                            results.lint = {
                                passed: toolUse.result.success,
            ...
            if (options.typeCheck !== false)
                enabledChecks.push('type checking');
            if (options.lint !== false)
                enabledChecks.push('linting');
            if (options.test !== false)
                enabledChecks.push('tests');
            if (options.coverage)
                enabledChecks.push('coverage');
            // Create task
            ...
            const allPassed = (results.typeCheck?.passed ?? true) &&
            ...
            spinner.fail('Error');
            console.error('Failed to run QA command:', error instanceof Error ? error.message : error);
            process.exit(1);
        }
    });
}
