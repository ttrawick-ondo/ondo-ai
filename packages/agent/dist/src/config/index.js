...
export async function loadConfig(workingDirectory, options = {}) {
    ...
    if (throwOnMissing) {
        throw new Error(`No configuration file found. Create one of: ${CONFIG_FILE_NAMES.join(', ')}`);
    }
...
async function getProjectName(workingDirectory) {
    try {
...
export async function writeConfig(workingDirectory, config, fileName = '.ondo-agent.json') {
    const filePath = path.resolve(workingDirectory, fileName);
    const content = JSON.stringify(config, null, 2) + '\n';
    await fs.writeFile(filePath, content, 'utf-8');
}
export async function initConfig(workingDirectory) {
    const projectName = await getProjectName(workingDirectory);
    const config = {
        project: {
            name: projectName,
