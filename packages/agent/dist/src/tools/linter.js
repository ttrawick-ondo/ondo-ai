...
export function createLinterTools(workingDirectory) {
    const runEslint = {
        name: 'runEslint',
        ...
        async execute(input) {
            const { paths = ['src'], fix = false, format = 'json', maxWarnings = -1, } = input;
            ...
            try {
                ...
                if (format === 'json') {
                    try {
                        ...
                        issues = parsed.flatMap((file) => file.messages.map((msg) => ({
                            file: file.filePath,
                            line: msg.line,
                            column: msg.column,
    ...
    const runTypeCheck = {
        ...
        async execute(input) {
            const { project = 'tsconfig.json' } = input;
    ...
    const formatCode = {
        ...
        async execute(input) {
            const { paths = ['src/**/*.{ts,tsx}'], check = false } = input;
    ...
    const runNextLint = {
        ...
        async execute(input) {
            const { fix = false, strict = false } = input;
    ...
    const checkImports = {
        ...
        async execute(input) {
            ...
            try {
                ...
                try {
                    const parsed = JSON.parse(result.stdout);
                    issues = parsed.flatMap((f) => f.messages
                        .filter((m) => m.ruleId?.includes('unused') || m.ruleId?.includes('unresolved'))
                        .map((m) => ({
                        file: f.filePath,
                        line: m.line,
                        column: m.column,
